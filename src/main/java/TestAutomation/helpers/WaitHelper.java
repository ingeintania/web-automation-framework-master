...
public class WaitHelper
{
	...
	public static void waitForElementToBeDisplayed(Config testConfig, WebElement element, String description)
	{
		...
		{
			WebDriverWait wait = new WebDriverWait(testConfig.driver, 100 * Long.parseLong(testConfig.getRunTimeProperty("ObjectWaitTime")));
			try
			{
				wait.until(ExpectedConditions.visibilityOf(element));
			}
			...
			{
				testConfig.logFailToEndExecution("Element '" + description + "' is visible even after waiting for " + (100 * +Long.parseLong(testConfig.getRunTimeProperty("ObjectWaitTime")) + " seconds"));
			}
		}
	}
